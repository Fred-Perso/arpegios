...
export default function ArpeggesPage() {
    ...
    <main className="min-h-screen bg-gray-900 text-white px-3 py-4">
      <div className="max-w-5xl mx-auto space-y-3">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-white">Arpèges Jazz</h1>
            <p className="text-xs text-gray-500">Visualisez les 7 arpèges de la gamme sur tout le manche</p>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-1 flex-wrap">
              <button onClick={() => setKeyNote(k => (k - 1 + 12) % 12)}
                className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-gray-700 active:bg-gray-600 text-gray-300 font-bold text-lg leading-none flex-shrink-0">‹</button>
        ...
        <div className="bg-gray-800 rounded-2xl p-3 sm:p-4">
          <Fretboard positions={positions} arpeggioType={chordType}
            fretMin={fretMin} fretMax={fretMax} showNotes={showNotes} variant="full" />
        </div>

        {/* 5 positions */}
        <div className="bg-gray-800 rounded-2xl p-3 sm:p-4">
          <PositionsPanel keyNote={keyNote} degreeIndex={degreeIndex}
            arpeggioType={chordType} mode={mode} showNotes={showNotes} />
        </div>

        {/* Favorites */}
        <FavoritesPanel keyNote={keyNote} degreeIndex={degreeIndex} chordName={chordName}
