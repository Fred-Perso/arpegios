...
export default function Fretboard({
  ...
  showNotes = false, mini = false, positionLabel,
}: Props) {
  const W = mini ? 200 : 720;
  const H = mini ? 110 : 200;
  const LEFT_MARGIN = mini ? 18 : 36;
  const RIGHT_MARGIN = mini ? 8 : 20;
  const TOP_MARGIN = mini ? 14 : 28;
  const BOT_MARGIN = mini ? 16 : 28;
  ...
  const dotR = mini ? 9 : 12;
  const fontSize = mini ? 7 : 9;

  return (
    <div className={mini ? '' : 'w-full overflow-x-auto'}>
      {positionLabel && (
        <p className="text-center text-xs text-gray-400 mb-1">{positionLabel}</p>
      )}
      <svg
        viewBox={`0 0 ${W} ${H}`}
        className={mini ? 'w-full' : 'w-full max-w-3xl mx-auto'}
        style={mini ? {} : { minWidth: 400 }}
      >
        {fretMin === 0 && (
        ...
        {FRET_MARKERS.filter(f => f >= fretMin && f <= fretMax).map(f => {
          ...
          return DOUBLE_FRET_MARKERS.includes(f) ? (
            <g key={f}>
              <circle cx={cx} cy={cy - stringH * 1.5} r={mini ? 3 : 5} fill="#374151" />
              <circle cx={cx} cy={cy + stringH * 1.5} r={mini ? 3 : 5} fill="#374151" />
            </g>
          ) : (
            <circle key={f} cx={cx} cy={cy} r={mini ? 3 : 5} fill="#374151" />
          );
        })}

        {!mini && Array.from({ length: numFrets }, (_, i) => (
          <text key={i}
        ...
        {mini && (
          <>
            <text x={LEFT_MARGIN + fretW * 0.5} y={H - 3}
              textAnchor="middle" fontSize={8} fill="#6b7280">{fretMin}</text>
            <text x={LEFT_MARGIN + (numFrets - 0.5) * fretW} y={H - 3}
              textAnchor="middle" fontSize={8} fill="#6b7280">{fretMax}</text>
          </>
        )}
        ...
        {positions.map((pos, i) => {
          ...
          const label = showNotes
            ? getNoteAt(pos.string, pos.fret)
            : noteLabels[pos.degreeIndex];
          return (
            <g key={i}>
          ...
          {noteLabels.map((label, i) => (
            <span key={i} className="flex items-center gap-1.5">
              <span className="w-4 h-4 rounded-full inline-block"
                style={{ background: DEGREE_FILL[i] }} />
              {label}
            </span>
          ))}
        </div>
