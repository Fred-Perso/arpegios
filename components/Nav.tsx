...
const STANDARDS_LINKS = [
  { href: '/standards/all-of-me',   label: 'All of Me'   },
  { href: '/standards/satin-doll',  label: 'Satin Doll'  },
];
...
const MAIN_LINKS = [
  ...
  { href: '/couleurs',       label: 'Couleurs',        icon: '🎨' },
...
export default function Nav() {
  ...
  const isStandards = path.startsWith('/standards');
    ...
    <>
      <nav className="bg-gray-900/95 backdrop-blur-sm border-b border-gray-800 px-4 py-2.5 sticky top-0 z-50">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-2">
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors
                  ${isStandards ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800'}`}>
                🎷 Standards
                <svg className={`w-3 h-3 opacity-50 transition-transform ${standardsOpen ? 'rotate-180' : ''}`}
              ...
              {standardsOpen && (
                <>
                  <div className="fixed inset-0 z-40" onClick={() => setStandardsOpen(false)} />
                  <div className="absolute top-full left-0 mt-1 bg-gray-800 border border-gray-700 rounded-2xl shadow-2xl py-2 z-50 w-52"
                    onClick={() => setStandardsOpen(false)}>
                    <p className="text-[10px] text-orange-400 uppercase tracking-wider px-3 pt-1 pb-1.5 font-bold">
                      Analyse · AlphaTab
                    </p>
                    {STANDARDS_LINKS.map(({ href, label }) => (
                      <Link key={href} href={href}
                    ...
                    <Link href="/standards"
                      className="block px-3 py-1.5 text-xs text-gray-500 hover:text-gray-300 transition-colors">
                      Tous les standards →
                    </Link>
                  </div>
                </>
          ...
          <div className="flex md:hidden items-center gap-2 flex-1 justify-end">
            ...
            {(isStandards && currentStdLabel) && (
              <span className="text-xs text-orange-400 font-semibold truncate max-w-[120px]">{currentStdLabel}</span>
            )}
            <button
      ...
      <div className={`fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-gray-900 border-l border-gray-700 z-50 md:hidden flex flex-col transition-transform duration-300 ease-out ${mobileOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto py-4 px-3">
          ...
          <p className="text-[10px] text-orange-400 uppercase tracking-widest font-bold px-2 mb-2">Standards · AlphaTab</p>
          <div className="space-y-0.5 mb-5">
            {STANDARDS_LINKS.map(({ href, label }) => (
              <Link key={href} href={href} onClick={() => setMobileOpen(false)}
                className={`flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm transition-colors
                  ${path === href ? 'bg-gray-700 text-white font-semibold' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}>
